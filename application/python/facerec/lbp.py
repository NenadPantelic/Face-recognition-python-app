...
class VarLBP(LocalDescriptor):
    ...
    def __call__(self,X):
        ...
        for i,p in enumerate(sample_points):
            # koordinate:
            y,x = p + (origy, origx)
            # aritmetika na ovim vrednostima:
            fx = np.floor(x)
            fy = np.floor(y)
            cx = np.ceil(x)
            cy = np.ceil(y)
            # razlomljeni deo vrednosti:
            ty = y - fy
            tx = x - fx
            ...
            N += w3*X[cy:cy+dy,fx:fx+dx]
            N += w4*X[cy:cy+dy,cx:cx+dx]
            # Variance calculation (http://en.wikipedia.org/wiki/Algorithms_for_calculating_variance#On-line_algorithm):
            delta = N - mean
