...
class AbstractDistance(object):
    ...
    def __call__(self,p,q):
        raise NotImplementedError("mora  __call__ metoda.")
...
class CosineDistance(AbstractDistance):
    """
	Ovo je inovativna matematicka metoda - Mahalanobis Kosinusna Distanca
...
class NormalizedCorrelation(AbstractDistance):
    """
        Racuna Normalizovani Koeficijent Korelacije dva vektora.
...
class BinRatioDistance(AbstractDistance):
    """
    Racuna the Bin Ratio Dissimilarity.
...
class L1BinRatioDistance(AbstractDistance):
    """
    Racuna L1-Bin Ratio Dissimilarity.
...
class ChiSquareBRD(AbstractDistance):
    """
    Racunanje Hi na kvadrat raspodele.
